...
(define arity 
 ...
 (initialise_arity_table 
  [adjoin 2 and 2 append 2 arity 1 assoc 2 boolean? 1 cd 1 compile 3 concat 2 cons 2 
   cons? 1 cn 2 declare 2 destroy 1 difference 2 do 2 dump 2 element? 2 empty? 1 interror 2 eval 1 eval-kl 1 explode 1 
...
(map (function systemf) 
      ...
      *home-directory* *version* *maximum-print-sequence-size* *printer* *macros* *os* *release* @v @p @s <- -> <e> == = >= > /. =!
      ...
      snd simple-error set save str run reverse remove read read-file read-file-as-bytelist 
      read-file-as-string read-byte quit put preclude preclude-all-but ps prolog? protect
      ...
      eval-kl eval error-to-string error empty? element? dump dumped do difference 
      destroy defun define defmacro defcc defprolog declare datatype cut cn cons? cons cond concat compile cd cases 
      call close bind bound? boolean? boolean bar! assoc arity append and adjoin 
